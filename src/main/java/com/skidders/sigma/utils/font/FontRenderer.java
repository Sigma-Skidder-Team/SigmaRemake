...
import com.skidders.sigma.utils.math.MathUtil;
...
public class FontRenderer {
    ...
    static {
        FontRenderer.RANDOM_OFFSET = 1;
    }

    public FontRenderer(ConcurrentLinkedQueue<FontData> textureQueue, Font font) {
        this(textureQueue, font, 256);
    }

    public FontRenderer(ConcurrentLinkedQueue<FontData> textureQueue, Font font, int characterCount) {
        this(textureQueue, font, characterCount, true);
    }

    public FontRenderer(ConcurrentLinkedQueue<FontData> textureQueue, Font font, boolean antiAlias) {
        this(textureQueue, font, 256, antiAlias);
    }

    public FontRenderer(ConcurrentLinkedQueue<FontData> textureQueue, Font font, int characterCount, boolean antiAlias) {
    ...
    private void ensureOpenGLContext() {
    ...
    private CharacterData[] setup(final CharacterData[] characterData, final int[] texturesIds, final ConcurrentLinkedQueue<FontData> textureQueue, final int type) {
        this.generateColors();
        ...
        for (int index = 0; index < characterData.length; ++index) {
            ...
            final BufferedImage characterImage = new BufferedImage(MathUtil.ceiling_double_int(width), MathUtil.ceiling_double_int(height), 2);
    ...
    private void createTexture(final int textureId, final BufferedImage image, final ConcurrentLinkedQueue<FontData> textureQueue) {
        ...
        buffer.flip();
        textureQueue.add(new FontData(textureId, image.getWidth(), image.getHeight(), buffer));
    }
    ...
    private int renderString(String text, float x, float y, final int color, final boolean shadow) {
        ...
        try {
            ...
            SigmaReborn.LOGGER.error("Couldn't render text");
        }
        ...
        for (int i = 0; i < length; ++i) {
            ...
            if (previous != '§') {
                if (character == '§') {
                    ...
                    if (obfuscated) {
                        character += (char) FontRenderer.RANDOM_OFFSET;
                    }
                    this.drawChar(character, characterData, x, y);
                    final CharacterData charData = characterData[character];
                    if (strikethrough) {
                        this.drawLine(new Vec2f(0.0f, charData.height / 2.0f), new Vec2f(charData.width, charData.height / 2.0f), 3.0f);
                    }
                    if (underlined) {
                        this.drawLine(new Vec2f(0.0f, charData.height - 15.0f), new Vec2f(charData.width, charData.height - 15.0f), 3.0f);
                    }
                    x += charData.width - 8.0f;
                }
            }
    ...
    private void drawLine(final Vec2f start, final Vec2f end) {
        GL11.glDisable(3553);
    ...
    private void drawLine(final Vec2f start, final Vec2f end, final float width) {
        GL11.glDisable(3553);
