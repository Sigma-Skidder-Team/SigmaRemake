...
public class InternalTextureLoader {
     ...
     */
    private Map<String, SoftReference<TextureImpl>> texturesLinear = new HashMap<>();
    /**
     * The table of textures that have been loaded in this loader
     */
    private Map<String, SoftReference<TextureImpl>> texturesNearest = new HashMap<>();
    /**
    ...
    public TextureImpl getTexture(InputStream in, String resourceName, boolean flipped, int filter, int[] transparent) throws IOException {
        Map<String, ?> hash = holdTextureData ? texturesLinear : texturesNearest;
        ...
        if (holdTextureData) {
            ...
            Map<String, TextureImpl> textureMap = (Map<String, TextureImpl>) hash;
            TextureImpl tex = textureMap.get(resName);
            if (tex != null) {
                return tex;
            }
        } else {
            // Cast to correct type
            @SuppressWarnings("unchecked")
            Map<String, SoftReference<TextureImpl>> textureMap = (Map<String, SoftReference<TextureImpl>>) hash;
            SoftReference<TextureImpl> ref = textureMap.get(resName);
            if (ref != null) {
                TextureImpl tex = ref.get();
                if (tex != null) {
                    return tex;
                } else {
                    textureMap.remove(resName);
                }
            }
        }
        ...
        try {
            ...
            throw new RuntimeException("Image-based resources must be loaded as part of init() or the game loop. They cannot be loaded before initialization.");
        }
        ...
        if (holdTextureData) {
            @SuppressWarnings("unchecked")
            Map<String, TextureImpl> textureMap = (Map<String, TextureImpl>) hash;
            textureMap.put(resName, tex); // Store as TextureImpl
        } else {
            @SuppressWarnings("unchecked")
            Map<String, SoftReference<TextureImpl>> textureMap = (Map<String, SoftReference<TextureImpl>>) hash;
            textureMap.put(resName, new SoftReference<>(tex)); // Store as SoftReference<TextureImpl>
        }
