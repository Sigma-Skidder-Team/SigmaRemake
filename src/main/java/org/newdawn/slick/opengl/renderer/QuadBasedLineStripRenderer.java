...
public class QuadBasedLineStripRenderer implements LineStripRenderer {
    ...
    private final float[] points, colors;
    private float width = 1.0F;

    public static int MAX_POINTS = 10000;
    private int pts, cpt;

    private boolean renderHalf, lineCaps = false, antialias;

    public QuadBasedLineStripRenderer() {
        this.points = new float[MAX_POINTS * 2];
        this.colors = new float[MAX_POINTS * 4];
    }
    ...
    public void vertex(float x, float y) {
        if (this.width != 1.0F) {
            ...
            this.pts++;
            int var5 = this.pts - 1;
            this.color(this.colors[var5 * 4], this.colors[var5 * 4 + 1], this.colors[var5 * 4 + 2], this.colors[var5 * 4 + 3]);
        } else {
            this.def.vertex(x, y);
        }
    }
    ...
    private void bindColor(int var1) {
        if (var1 < this.cpt) {
            if (!this.renderHalf) {
                this.GL.glColor4f(this.colors[var1 * 4], this.colors[var1 * 4 + 1], this.colors[var1 * 4 + 2], this.colors[var1 * 4 + 3]);
            } else {
                this.GL
                        .glColor4f(
                                this.colors[var1 * 4] * 0.5F,
                                this.colors[var1 * 4 + 1] * 0.5F,
                                this.colors[var1 * 4 + 2] * 0.5F,
                                this.colors[var1 * 4 + 3] * 0.5F
                        );
            }
        }
    }
    ...
    public void color(float r, float g, float b, float a) {
        if (this.width != 1.0F) {
            this.colors[this.pts * 4] = r;
            this.colors[this.pts * 4 + 1] = g;
            this.colors[this.pts * 4 + 2] = b;
            this.colors[this.pts * 4 + 3] = a;
            this.cpt++;
        } else {
