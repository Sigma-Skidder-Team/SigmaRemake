...
public class ProfileListEntry extends Widget {
    ...
    public ProfileListEntry(GuiComponent parent, String name, int x, int y, int width, int height, Profile profile) {
        ...
        ProfileActionButton renameButton = new ProfileActionButton(
                this.editButtons,
                "rename",
                0,
                0,
                this.editButtonsWidth / 2,
                height,
                renameButtonColors,
                "Rename"
        );
        this.editButtons.addToList(renameButton);

        ProfileActionButton deleteButton = new ProfileActionButton(
                this.editButtons,
                "remove",
                this.editButtonsWidth / 2,
                0,
        ...
        this.editButtons.addToList(deleteButton);
        ...
        this.profileName = new TextField(this, "profileName", 16, 8, this.getWidth() - 60, 50, textFieldColor, profile.name);
        this.addToList(this.profileName);
        ...
        this.profileName.addKeyPressListener((character, key) -> {
            if (this.profileName.isFocused() && key == GLFW.GLFW_KEY_ENTER) {
                this.renameProfile(this.profileName.getText());
            }
        });
        ...
        renameButton.addWidthSetter((button, editPanel) -> button.setWidth(Math.round((float) editPanel.getWidth() / 2.0F)));
        deleteButton.addWidthSetter((button, editPanel) -> {
            button.setX(Math.round((float) editPanel.getWidth() / 2.0F));
            button.setWidth(Math.round((float) editPanel.getWidth() / 2.0F));
        });
        ...
        this.onClick((mouseX, mouseButton) -> {
            if (mouseButton != 1) {
                this.slideAnimation.changeDirection(AnimationUtils.Direction.FORWARDS);
    ...
    public void updatePanelDimensions(int mouseX, int mouseY) {
        ...
        this.hoverAnimation.changeDirection(
                this.isMouseOverComponent(mouseX, mouseY)
    ...
    public void draw(float partialTicks) {
        float deleteCollapse = VecUtils.interpolate(this.deleteAnimation.calcPercent(), 0.1, 0.81, 0.14, 1.0);
        this.setHeight(Math.round((1.0F - deleteCollapse) * (float) this.initialHeight));
        ...
        this.editButtons.setWidth(Math.max(0, (int) ((float) this.editButtonsWidth * slidePercent)));
        this.editButtons.setTranslateX((int) ((float) this.editButtonsWidth * (1.0F - slidePercent)));
        ...
        this.profileName.setTranslateX(Math.round(-slidePercent * (float) this.width));

        if (Client.INSTANCE.configManager.currentProfile == this.profile) {
            RenderUtils.drawImage(
                    (float) (this.getX() + this.getWidth() - 35) - slidePercent * (float) this.width,
                    (float) (this.getY() + 27),
                    17.0F,
                    13.0F,
