...
public class OldHypixelBlockFly extends SubModule {
    ...
    public void onRotate(RotateEvent event) {
        ...
        if (pendingPlace != null) {
            ...
            if ((double) pendingPlace.blockPos.getY() - client.player.getY() < 0.0) {
                double dx = client.player.getX()
                        - (pendingPlace.blockPos.getX() + 0.5 + pendingPlace.direction.getOffsetX() / 2.0);
                double dz = client.player.getZ()
